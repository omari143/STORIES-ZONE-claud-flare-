// /api/audio-preview.js
// Hutengeneza Open Graph meta tags kwa podcast maalum kwa kutumia SLUG
...
export default async function handler(req, res) {
  ...
  if (!slug) {
    res.status(400).send('Missing audio slug');
    return;
  }

  try {
    const url = `${SUPABASE_URL}/rest/v1/audio_items?slug=eq.${encodeURIComponent(slug)}&select=title,description,cover_image_url,slug&is_published=eq.true&limit=1`;
    ...
    const audio = Array.isArray(rows) && rows[0];
    ...
    const title = `${audio.title} — Stories Zone`;
    const description = audio.description
      ? String(audio.description).slice(0, 160)
      : 'Sikiliza simulizi za sauti kwenye Stories Zone';
    const image =
      audio.cover_image_url ||
      'https://images.unsplash.com/photo-1507842217343-583bb7270b66?w=1200&q=80';

    const pageUrl = `https://stories-zone.vercel.app/audio/${audio.slug}`;
